...
export default function Page() {
  ...
  const [weekStartDate, setWeekStartDate] = useState(() => {
    ...
    //  distanation tosaturday
    let distToSat = (dayIndex + 1) % 7;
    let sat = new Date();
  // sunday =0 / sat =6 // getDate daynumber from the month 
  ...
  let filteredAppointmentsThisWeek = appointmentsThisWeek.filter(
    (appointment) => {
  ...
  return (
    <div className=" w-full h-full p-8 ">
      <div className="upper w-full" dir="rtl">
        <div className="up w-full  flex flex-col md:flex-row justify-between  items-start gap-2 md:gap-0 mb-6">
          {' '}
          <p className="  text-[#314158] font-bold  lg:text-[20px] text-[16px] ">
            اختر التاريخ
          </p>
          ...
          {currentWeekDays.map((day) => {
            ...
            return (
              <button
                ...
                key={day.fullDate}
                className={`px-4 py-3 flex flex-col gap-1 border border-gray-300 hover:shadow-md hover:border-0 transition duration-100 hover:transition-transform hover:scale-102  rounded-xl  ${isSelected ? ' border-0 transition-transform scale-105 bg-[#ecf5f6] text-[#0D9488] shadow-lg ' : 'text-[#62748E] bg-white '}`}
              >
                {day.dayName}
                <span
                  className={`${isSelected ? 'text-[#139C89]' : 'text-[#0F172B]'} font-bold`}
                >
                  {' '}
                  {day.dayNumber}
                </span>
        ...
        <div className="buttons border border-[#E2E8F0] md:flex md:flex-row bg-white rounded-lg h-13 shadow-md w-full md:w-fit flex py-1 relative gap-1 px-2">
          <div
            className={`absolute w-1/4 transition-all duration-300 ease-in-out m-1 ml-3 top-1 rounded-lg h-9 bg-[#e7f5f4] font-bold ${
              appointmentStatus === 'مكتمل'
                ? 'right-0'
                : appointmentStatus === 'في الانتظار'
                  ? 'right-[25%]'
                  : appointmentStatus === 'مجدول'
                    ? 'right-[47%]'
                    : 'right-[72%]'
            }`}
          ></div>
          <button
            onClick={() => {
              setAppointmentStatus('مكتمل');
              setCurrentPage(1);
            }}
            className={`z-10 ${filterButtonStyle} ${appointmentStatus === 'مكتمل' ? 'text-[#0d9488] font-bold' : 'text-[#45556C]'}`}
          >
            مكتمل
          </button>
          <button
            onClick={() => {
              setAppointmentStatus('في الانتظار');
              setCurrentPage(1);
            }}
            className={`whitespace-nowrap z-10 ${filterButtonStyle} ${appointmentStatus === 'في الانتظار' ? 'text-[#0d9488] font-bold' : 'text-[#45556C]'}`}
          >
            انتظار
          </button>
          <button
            onClick={() => {
              setAppointmentStatus('مجدول');
              setCurrentPage(1);
            }}
            className={`z-10 ${filterButtonStyle} ${appointmentStatus === 'مجدول' ? 'text-[#0d9488] font-bold' : 'text-[#45556C]'}`}
          >
            مجدول
          </button>
          <button
            onClick={() => {
              setAppointmentStatus('الكل');
              setCurrentPage(1);
            }}
            className={`z-10 ${filterButtonStyle} ${appointmentStatus === 'الكل' ? 'text-[#0d9488] font-bold' : 'text-[#45556C]'}`}
          >
            الكل
          </button>
        </div>
          ...
          <tbody className="divide-y divide-[#F1F5F9] text-sm">
            {paginatedAppointments.map((appointment) => (
              <tr key={appointment.id} className="hover:bg-slate-50 transition">
                <td className="py-4 px-6 text-center font-bold text-[#0F172B] whitespace-nowrap">
                  {new Date(
                ...
                <td className="py-4 px-6 text-right">
                  <div className="flex items-center gap-3">
                    <span className="font-bold text-[#0F172B] whitespace-nowrap ">
                      {appointment.patientName}
                    </span>
                  </div>
                </td>
                <td className="py-4 px-6 text-center text-[#45556C]">
                  {appointment.procedure}
                </td>
                <td className="py-4 px-6 text-center">
                  <span
                    className={` whitespace-nowrap px-2 text-[11px] py-0.5 font-bold shadow-md rounded-full outline-none ${
                ...
                <td className="py-4 px-6 text-center">
                  <button
                    className="text-[#90A1B9] duration-200 transition-transform  hover:scale-102 hover:text-gray-500 "
                    onClick={() => handleEdit(appointment.patientId)}
                  >
                    <Eye />
                  </button>
                </td>
