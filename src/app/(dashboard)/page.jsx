...
export default function page() {
  ...
  return (
    <div className=" w-full h-full md:p-8 p-3 gap-8 flex flex-col ">
      {/* Top */}
      <p className=" w-full font-bold text-[25px] " dir="rtl">
        أهلا يا {currentUser?.fullName}
      </p>
      <div className="md:grid flex flex-col-reverse  md:grid-cols-3 w-full gap-4 md:gap-6   ">
        ...
        <div className={`  ${oneOfTheGrid} text-[#155DFC] `}>
          <div className="w-full flex justify-between items-center ">
            <p className=" text-[12px] font-bold bg-[#eff6ff] px-1 py-0.75 rounded-full ">
              مكتمل
            </p>
            <CheckCircle2 className="text-[#155DFC] rounded-lg bg-[#eff6ff] h-10 w-10 p-2" />
          </div>
          <p className=" text-[16px] text-[#62748e] text-end mt-3 mb-1 font-medium">
            مكتمله اليوم
          </p>
          <p className=" text-end  text-black font-bold text-[30px]">
            {numberOfTheAppointmentsCompeletedToday}
          </p>
        </div>
        {/* appointments of the day */}
        <div
          className={oneOfTheGrid}
          onClick={() => {
            router.push('/appointments');
          }}
        >
          <div className="w-full flex justify-between items-center ">
            <p className=" text-[12px] font-bold text-[#90A1B9] px-1.5 py-0.75 rounded-full bg-[#F8FAFC]">
              اليوم
            </p>
        ...
        >
          ...
          <p className=" text-[16px] text-[#62748E] text-end mt-3 mb-1 font-medium whitespace-nowrap">
            إجمالي المرضى
          </p>
          <p className=" text-end  text-black font-bold text-[30px]">
            {patients.length}
      ...
      <div className="appointmentsTable md:w-full   ">
        <div className="appointmentsTable md:w-full shadow-lg bg-white  overflow-auto flex flex-col grow h-auto rounded-lg ">
          <div
            className="head w-full justify-between flex items-center px-2  md:px-6 rounded-t-lg   py-3 "
            dir="rtl"
          >
            <p className="text-[#0F172B] text-lg font-bold whitespace-nowrap text-[16px]">
              مواعيد اليوم القادمة
            </p>
            <Link
              href="/appointments"
              className="text-[#0D9488] font-bold text-sm whitespace-nowrap text-[16px]"
            >
              عرض كل المواعيد ←
            </Link>
          </div>
          <table
            dir="rtl"
            className="bg-white shadow-md w-full   overflow-hidden   border-r border-b border-l border-[#E2E8F0] rounded-b-xl text-right "
          >
            <thead>
              <tr dir="rtl" className="     "></tr>
            ...
            <tbody>
              {paginatedData?.map((appointment) => {
                return (
                  ...
                  >
                    <td className=" text-[#0F172B] text-[10px] md:text-[18px]  font-bold py-5  text-center md:px-6 ">
