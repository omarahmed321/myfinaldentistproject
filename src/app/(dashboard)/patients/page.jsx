...
export default function page() {
  ...
  const {
    ...
    to,
    total,
  } = paginate(filteredItems, currentPage, 5);
      ...
      <div className="text-black  flex ">
        <div className=" w-full      flex flex-col md:flex-row-reverse gap-5 lg:gap-0 ">
          ...
          <div className="buttons border border-[#E2E8F0]  md:flex md:flex-row bg-white rounded-lg h-13 shadow-md w-full md:w-fit flex   py-1 relative gap-1 px-2">
            ...
            <button
              onClick={() => {
                setbuttonStatus('جديد');
                setCurrentPage(1);
              }}
              className={` z-10 ${filterButtonStyle} ${buttonStatus == 'جديد' ? '  text-[#0d9488] font-bold ' : 'text-[#45556C]'} `}
            >
              جديد
            </button>
            <button
              onClick={() => {
                setbuttonStatus('منتظم');
                setCurrentPage(1);
              }}
              className={` z-10 ${filterButtonStyle} ${buttonStatus == 'منتظم' ? ' text-[#0d9488] font-bold' : 'text-[#45556C]'}`}
            >
              منتظم
            </button>
            <button
              onClick={() => {
                setbuttonStatus('يحتاج متابعه');
                setCurrentPage(1);
              }}
              className={` whitespace-nowrap  z-10 ${filterButtonStyle} ${buttonStatus == 'يحتاج متابعه' ? ' text-[#0d9488] font-bold' : 'text-[#45556C]'}`}
            >
              متابعه
            </button>
            <button
              onClick={() => {
                setbuttonStatus('الكل');
                setCurrentPage(1);
              }}
              className={` z-10 ${filterButtonStyle} ${buttonStatus == 'الكل' ? ' text-[#0d9488] font-bold' : 'text-[#45556C]'}`}
            >
              الكل
            </button>
          </div>
        ...
        <table className=" border border-[#F1F5F9] w-full text-right shadow-lg rounded-lg  lg:table-fixed border-separate border-spacing-0 tab overflow-hidden text-sm md:text-[18px] lg:text-[20px]">
          <thead className=" bg-[#f8fafc] rounded-lg ">
            <tr className=" rounded-lg">
              ...
              <th className={` whitespace-nowrap ${headStyle} w-[20%]`}>
                رقم الهاتف
              </th>
              ...
              return (
                ...
                >
                  ...
                  <td className="font-bold text-xs pr-5 py-2">
                    {el.status ? (
                      <span
                        className={` shadow-md whitespace-nowrap ${el.status === 'جديد' ? 'text-[#1447E6] bg-[#EFF6FF] ' : ''} ${el.status === 'يحتاج متابعه' ? 'text-[#BB4D00] bg-[#FFFBEB] ' : ''} ${el.status === 'منتظم' ? 'text-[#008236] bg-[#f0fdf4] ' : ''}  py-0.5 px-2.5  rounded-2xl  w-[15%]`}
                      >
                        {el.status}
                      </span>
                    ) : (
                  ...
                  <td
                    onClick={() => handlePatientDetails(el.id)}
                    className="whitespace-nowrap    cursor-pointer hover:transition-transform hover:scale-103  transition duration-300 font-semibold text-[#0F172B] pr-6 text-[9px] md:text-[16px] lg:text-[20px]"
                  >
                    <div className=" w-full justify-end  items-center flex  gap-3">
                      {' '}
                      {el.name}{' '}
                      <p className=" whitespace-nowrap p-2 text-[#45556C] bg-[#F1F5F9] md:text-[16px]     rounded-full flex  justify-center items-center text-center text-[10px]  ">
                        {(() => {
                          let nameParts = el.name
                            ? el.name.trim().split(' ')
                            : [];
